...
if __name__ == '__main__':
    ...
    parser.add_argument("-ad", "--density_A", type=float, default=0.1, help="Density of intermediate connection")
    ...
    if args.output == "":
        args.output = os.path.join("gcn/toy/outputs", args.model_type, "ic{}dA{}s{}".format(args.intermediate_channel, args.density_A, args.seed), args.datafile.replace(".npz", ".chk"))
    ...
    if args.model_type == 'baseline':
        As = None
        ks = [np.prod(dset.xdim), 256 * args.intermediate_channel, dset.ydim[0]]
    else:
        ...
        A1 = sp.rand(A0.shape[0], dset.ydim[0], args.density_A).T  # k * ydim[0]
        As = [utils.scsp2tsp(A0), utils.scsp2tsp(A1)]
        ks = [3, args.intermediate_channel, 1]
    ...
    torch.save(
        checkpoint,
        os.path.join("gcn/outputs/{}".format(args.datafile.replace(".npz", args.model_type + "_" + str(seed)))))
    del checkpoint
    # Testing
