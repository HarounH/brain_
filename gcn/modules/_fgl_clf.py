...
class Classifier1(nn.Module):
    def __init__(self, args, loadable_state_dict=None, z_size=128, dropout_rate=0.5, downsampled=False):
        ...
        # Over fits with:
            ...
            # self.channel_sizes = [1, 32, 64, 128]
        # 73% test acc with:
        ...
        FGL = fgl.versions[args.fgl_version]  # Either v1 or v2 depending on args.
        self.downsample0 = weight_norm(FGL(int(self.channel_sizes[0]), int(self.node_sizes[0]), int(self.channel_sizes[1]), int(self.node_sizes[1]), adj_list[0], must_use_padded=args.no_opt))
        self.downsample1 = weight_norm(FGL(int(self.channel_sizes[1]), int(self.node_sizes[1]), int(self.channel_sizes[2]), int(self.node_sizes[2]), adj_list[1], must_use_padded=args.no_opt))
        self.downsample2 = weight_norm(FGL(int(self.channel_sizes[2]), int(self.node_sizes[2]), int(self.channel_sizes[3]), int(self.node_sizes[3]), adj_list[2], must_use_padded=args.no_opt))
        ...
        )  # nn.Sequential(nn.LeakyReLU(0.2), nn.BatchNorm1d(self.channel_sizes[1]))  # nn.Sequential(nn.LeakyReLU(0.2), nn.Dropout(0.5))
        self.activation1 = nn.Sequential(
            # nn.BatchNorm1d(self.channel_sizes[2]),
        )  # nn.Sequential(nn.LeakyReLU(0.2), nn.BatchNorm1d(self.channel_sizes[2]))  # nn.Sequential(nn.LeakyReLU(0.2), nn.Dropout(0.5))
        self.activation2 = nn.Sequential(
            # nn.BatchNorm1d(self.channel_sizes[3]),
            # nn.Dropout(dropout_rate)
        )  # nn.Sequential(nn.LeakyReLU(0.2), nn.BatchNorm1d(self.channel_sizes[3]))  # nn.Sequential(nn.LeakyReLU(0.2), nn.Dropout(0.5))
