...
class BigResolutionGeneratorHierarchical0(nn.Module):
    def __init__(self, args, mask=None, loadable_state_dict=None, z_size=128, content_channels=16, dropout_rate=0.5):
        super(BigResolutionGeneratorHierarchical0, self).__init__()
        self.args = args
        meta = self.args.meta
        self.z_size = z_size
        self.content_channels = content_channels
        ...
        # (1, 2, 1) - k343s2p010 - (3, 4, 3) - k4s2p1 - (6, 8, 6) - k343s2p010 - (13, 16, 13) - k4s2p1 - (26, 32, 26) - k344s2p011 - 53, 64, 52
        # Now, to convolve.
        self.upsample0 = CCT3D((2, 3, 2), z_size + content_channels, z_size // 2, (3, 4, 3), stride=2, padding=(0, 1, 0), use_spectral_norm=False)
        self.activation0 = nn.Sequential(nn.LeakyReLU(0.2), nn.BatchNorm3d(z_size // 2))

        self.upsample1 = CCT3D((5, 6, 5), z_size // 2 + content_channels, z_size // 4, (3, 3, 3), stride=2, padding=(0, 0, 0), use_spectral_norm=False)
        self.activation1 = nn.Sequential(nn.LeakyReLU(0.2), nn.BatchNorm3d(z_size // 4))
        # self.residual1 = ResidualBlock((11, 13, 11), z_size // 4, [3], use_spectral_norm=False)

        self.upsample2 = CCT3D((11, 13, 11), z_size // 4 + content_channels, z_size // 8, (4, 3, 4), stride=2, padding=(1, 0, 1), use_spectral_norm=False)
        self.activation2 = nn.Sequential(nn.LeakyReLU(0.2), nn.BatchNorm3d(z_size // 8))

        self.upsample3 = CCT3D((22, 27, 22), z_size // 8 + content_channels, z_size // 16, (3, 4, 3), stride=2, padding=(0, 1, 0), use_spectral_norm=False)
        # self.residual3 = ResidualBlock((45, 54, 45), z_size // 16, [3], use_spectral_norm=False)
        self.activation3 = nn.Sequential(nn.LeakyReLU(0.2), nn.BatchNorm3d(z_size // 16))

        self.upsample4 = CCT3D((45, 54, 45), z_size // 16 + content_channels, 1, (3, 3, 3), stride=2, padding=(0, 0, 0), use_spectral_norm=False)
        self.activation4 = nn.Sequential(nn.Tanh())
    ...
    def forward(self, z, studies, tasks, contrasts):
        ...
        # Now, add the contents to z as it goes through the CNN
        cur_z = self.zfc(z).unsqueeze(2).unsqueeze(2).unsqueeze(2).expand(z.shape[0], z.shape[1], 2, 3, 2)
        for i in range(0, 5):
            upsample = getattr(self, 'upsample{}'.format(i))
            content = contents[i].unsqueeze(2).unsqueeze(2).unsqueeze(2).expand(z.shape[0], self.content_channels, cur_z.shape[2], cur_z.shape[3], cur_z.shape[4])
            cur_z = upsample(torch.cat([cur_z, content], dim=1))
...
versions = {
    ...
    'big0': BigResolutionGeneratorHierarchical0,
}


infer_downsampled = defaultdict(lambda: False)
infer_downsampled['0'] = True
infer_downsampled['1'] = True
