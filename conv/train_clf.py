...
def get_args():
    ...
    # outputs
    parser.add_argument("--base_output", dest="base_output", default="conv/outputs/clf/", help="Directory which will have folders per run")  # noqa
    ...
    parser.add_argument("-clft", "--classification_weight_task", dest="clft", default=0.5, type=float, help="Weight to use on classification loss of task. Tweak to avoid class dependency")
    parser.add_argument("-clfs", "--classification_weight_study", dest="clfs", default=0.25, type=float, help="Weight to use on classification loss of study. Tweak to avoid class dependency")
    ...
    with torch.no_grad():
        for s, loader in loaders.items():
            ...
            for bidx, (x, svec, tvec, cvec) in enumerate(loader):
                ...
                losses["study acc"].append((torch.argmax(spred.detach(), dim=1) == svec).float().mean().item())
                losses["study ce"].append(F.cross_entropy(spred, svec).item())
                losses["task acc"].append((torch.argmax(tpred.detach(), dim=1) == tvec).float().mean().item())
                losses["task ce"].append(F.cross_entropy(tpred, tvec).item())
...
def train(args, train_datasets, train_loaders, test_datasets, test_loaders, model, writer, prefix="train", checkpoint=None):
    ...
    for batch_idx, batch in enumerate(utils.multi_key_infinite_iter(train_loaders)):
        ...
        for study, (x, svec, tvec, cvec) in batch_items:
            ...
            losses["s acc"] = acc_s
            losses["s ce"] = L_clf_s.item()
            losses["t acc"] = acc_t
            losses["t ce"] = L_clf_t.item()
            losses["c acc"] = acc_c
            losses["c ce"] = L_clf_c.item()
            optimizer.zero_grad()
            loss.backward()
            for pn, p in model.named_parameters():
                if p.requires_grad and not("bias" in pn):
                    losses["norm-{}".format(pn)] = p.detach().norm().item()
                    losses["norm-grad-{}".format(pn)] = p.grad.detach().norm().item()
